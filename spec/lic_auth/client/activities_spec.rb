...
describe LicAuth::ClientCredentials do
  context "retrieving activities for a token" do
    ...
    it "retrieves activities for a valid JWT" do
      allow(LicAuth::Client).to receive(:get).with("/identity/activities", base_uri: nil, query: { id_token: "valid_token" }).and_return(double(body: activities_hash.to_json))
    ...
    it "returns an empty list for an invalid JWT" do
      ...
      allow(LicAuth::Client).to receive(:get).with("/identity/activities", base_uri: nil, query: { id_token: "invalid_token" }).and_raise(auth_error)
